...
def test_time_offset_and_window() -> None:
    ...
    highlight_line = next(line for line in dialogue_lines if highlight_tag in line)
    times = _parse_ass_times(highlight_line)
    word_start = _ass_time_to_seconds(times[0])
    word_end = _ass_time_to_seconds(times[1])
...
def test_step_events_have_no_overlap() -> None:
    ...
    doc = _build_word_timing_doc(
        ...
        cue_text="שלום עולם יפה",
        words=[
            WordSpan(text="שלום", start=0.05, end=0.25),
            WordSpan(text="עולם", start=0.3, end=0.6),
            WordSpan(text="יפה", start=0.7, end=0.9),
        ],
    )
    style = build_style_config_from_subtitle_style(
        preset_defaults(PRESET_DEFAULT),
    ...
    dialogue_lines = [line for line in ass_text.splitlines() if line.startswith("Dialogue:")]
    times = [
        (_ass_time_to_seconds(start), _ass_time_to_seconds(end))
        for start, end, _ in (_parse_ass_times(line) for line in dialogue_lines)
    ]
    times_sorted = sorted(times, key=lambda pair: pair[0])
