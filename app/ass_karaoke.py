...
def _build_step_highlight_text(
    ...
    for idx, part in enumerate(word_parts):
        color_tag = highlight_tag if word_index == idx else normal_tag
...
def build_ass_step_highlight_document_with_stats(
    ...
    *,
    ...
    for cue in normalized:
        ...
        for word_index in range(limit):
            ...
            if end <= start:
                skipped += 1
                continue
            word_entries.append((start, end, word_index))

        if not word_entries:
            continue

        segments: list[_Segment] = []
        first_start, _, _ = word_entries[0]
        if cue.start_sec < first_start:
            segments.append(
                _Segment(
                    start_s=cue.start_sec,
                    end_s=first_start,
                    word_index=None,
                    is_highlight=False,
                )
        ...
        for segment in segments:
            ...
            start_cs = _floor_centiseconds(event_start)
            end_cs = _ceil_centiseconds(event_end)
            if end_cs <= start_cs:
                end_cs = start_cs + 1
            rounded_segments.append((start_cs, end_cs, segment.word_index, segment.is_highlight))

        cleaned_segments: list[tuple[int, int, Optional[int], bool]] = []
        for idx, (start_cs, end_cs, word_index, is_highlight) in enumerate(rounded_segments):
            if end_cs <= start_cs:
                continue
            duration = end_cs - start_cs
            if duration < 1:
                if cleaned_segments and cleaned_segments[-1][3] is False:
                    prev_start, prev_end, prev_word, prev_highlight = cleaned_segments[-1]
                    cleaned_segments[-1] = (prev_start, max(prev_end, end_cs), prev_word, prev_highlight)
                    continue
                if idx + 1 < len(rounded_segments) and rounded_segments[idx + 1][3] is False:
                    next_start, next_end, next_word, next_highlight = rounded_segments[idx + 1]
                    rounded_segments[idx + 1] = (min(next_start, start_cs), next_end, next_word, next_highlight)
                    continue
            cleaned_segments.append((start_cs, end_cs, word_index, is_highlight))

        last_end_cs = None
        for start_cs, end_cs, word_index, is_highlight in cleaned_segments:
            if last_end_cs is not None and start_cs < last_end_cs:
                overlap_detected = True
                start_cs = last_end_cs
            if end_cs <= start_cs:
                continue
            payload = _build_step_highlight_text(
                prefix,
                word_parts,
                word_index if is_highlight else None,
                normal_color_ass,
                highlight_color_ass,
            )
            events.append((start_cs / 100, 0, end_cs / 100, payload))
            if is_highlight:
                highlight_events += 1
            else:
                no_highlight_events += 1
            last_end_cs = end_cs
    ...
    LOG.info(
        "step_segments_total=%s highlight_segments=%s no_highlight_segments=%s overlap_detected=%s",
        highlight_events + no_highlight_events,
        highlight_events,
        no_highlight_events,
        str(overlap_detected).lower(),
    )
